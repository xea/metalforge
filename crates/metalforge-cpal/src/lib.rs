...
pub fn run_demo() {
    ...
    let stream_in = match config_in.sample_format() {
        SampleFormat::F32 => device_in.build_input_stream(
            &config_in.into(),
            data_callback_f32,
            error_callback,
            None)
...
fn data_callback<T>(data: &[T], _input_callback_info: &InputCallbackInfo) where T: Sample {
...
fn handle_input<T, U>(input: &[T]) where T: Sample, U: Sample + FromSample<T> {
...
fn find_best_config(device: &Device) -> SupportedStreamConfig {
    let _default_config = device.default_input_config().expect("Failed to get default configuration");
    let supported_configs = device.supported_input_configs().expect("Failed to get supported configurations");
